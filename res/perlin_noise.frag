...
void main(void)
{
    ...
    vec3 result = (ambient + diffuse + specular) * vert_pos_smooth;

    float dist = distance(viewPos, frag_pos);

    float opacity = clamp(dist / 50.0f, 0.0f, 1.0f) * fade;

    float n = cnoise(vec4(vert_pos * scale, time));

    colour = vec4(result * n, 1.0 - opacity);
}